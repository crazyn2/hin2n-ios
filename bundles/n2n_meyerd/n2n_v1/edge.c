...
#endif
#else /* defined(__ANDROID_NDK__) || defined(__APPLE__) */
#ifdef __ANDROID_NDK__
int start_edge_v1(n2n_edge_status_t* status) {
#endif
#ifdef __APPLE__
__attribute__((visibility("default"))) int start_edge_v1(CurrentSettings *settings) {
#endif
    u_int16_t local_port = 0 /* any port */;
    ...
    {
        strncpy(ip_addr, settings->ipAddress, N2N_NETMASK_STR_SIZE);
        ip_addr[N2N_NETMASK_STR_SIZE - 1] = '\0';
    }
#endif
    else
    ...
    {
        eee.community_name = settings->community;
    }
#endif
    else
    {
    ...
    {
        strncpy(device_mac, settings->mac, sizeof(device_mac));
        device_mac[sizeof(device_mac) - 1] = '\0';
    }
#endif
    else
    ...
    {
        strncpy(eee.supernode_ip, settings->supernode, N2N_EDGE_SN_HOST_SIZE);
        eee.supernode_ip[N2N_EDGE_SN_HOST_SIZE - 1] = '\0';
    }
#endif
    else
...
#endif
#ifdef __APPLE__
    if(tuntap_open(&(eee.device), tuntap_dev_name, ip_addr, netmask, device_mac, settings->mtu) < 0)
#endif
    {
...
#if defined(__ANDROID_NDK__) || defined(__APPLE__)
__attribute__((visibility("default"))) int stop_edge_v1(void)
{
    keep_running = 0;
