...
// Note : MUST place before including n2n.h or it will include n2n_android_t struct,
// which is including some struct only in v2 and v3 code
#ifdef __APPLE__
#include "libs_def.h"
...
#if defined(__ANDROID_NDK__) || defined(__APPLE__)
    if (uip_arp_len != 0) {
        len = uip_arp_len;
...
#else /* #if defined(__ANDROID_NDK__) || defined(__APPLE__) */
#ifdef __ANDROID_NDK__
int start_edge_v2s(n2n_edge_status_t* status)
#endif
#ifdef __APPLE__
__attribute__((visibility("default"))) int start_edge_v2s(CurrentSettings *settings)
#endif
{
    int     local_port = 0 /* any port */;
    ...
    slog = initslog(android_log_level(traceLevel), settings->logPath);
#endif
    ...
    // TODO  APP暂时没有加密文件的设置项，这里暂时留空
    
    if (settings->encryptKey && settings->encryptKey[0] != '\0')
    {
        encrypt_key = strdup(settings->encryptKey);
        traceEvent(TRACE_DEBUG, "encrypt_key = '%s'\n", encrypt_key);
    }
#endif
...
#endif
#ifdef __APPLE__
    if (settings->ipAddress && settings->ipAddress[0] != '\0')
    {
        scan_address(ip_addr, N2N_NETMASK_STR_SIZE,
                     ip_mode, N2N_IF_MODE_SIZE,
                     settings->ipAddress);
...
#endif
#ifdef __APPLE__
    if (settings->community && settings->community[0] != '\0')
    {
        strncpy((char *)eee.community_name, settings->community, N2N_COMMUNITY_SIZE);
    }
#endif
    else
    {
...
#endif
#ifdef __APPLE__
    eee.allow_routing = settings->forwarding == 0 ? 0 : 1;
#endif
...
#endif
#ifdef __APPLE__  // TODO 暂时就先支持2个supernode
    if (settings->supernode && settings->supernode[0] != '\0')
    {
        strncpy(eee.sn_ip_array[eee.sn_num], settings->supernode, N2N_EDGE_SN_HOST_SIZE);
        traceEvent(TRACE_DEBUG, "Adding supernode[%u] = %s\n", (unsigned int)eee.sn_num, (eee.sn_ip_array[eee.sn_num]));
        ++eee.sn_num;
    }
    if (settings->supernode2 && settings->supernode2[0] != '\0')
    {
        strncpy(eee.sn_ip_array[eee.sn_num], settings->supernode2, N2N_EDGE_SN_HOST_SIZE);
        traceEvent(TRACE_DEBUG, "Adding supernode[%u] = %s\n", (unsigned int)eee.sn_num, (eee.sn_ip_array[eee.sn_num]));
        ++eee.sn_num;
    }
#endif
...
#endif
#ifdef __APPLE__
    // TODO APP暂缺参数，这里暂时设置为15秒
    eee.holepunch_interval = 15;
#endif
    eee.holepunch_interval = MAX(eee.holepunch_interval, REGISTER_SUPER_INTERVAL_MIN);
    eee.holepunch_interval = MIN(eee.holepunch_interval, REGISTER_SUPER_INTERVAL_MAX);
    ...
    {
        eee.local_sock_ena = 1;
        strncpy(eee.local_ip_str, settings->ipAddress, N2N_EDGE_LOCAL_IP_SIZE);
        if (N2N_EDGE_LOCAL_IP_SIZE > 0)
        {
            eee.local_ip_str[N2N_EDGE_LOCAL_IP_SIZE - 1] = '\0';
        }
...
#endif
#ifdef __APPLE__
    if(tuntap_open(&(eee.device), tuntap_dev_name, ip_mode, ip_addr, netmask, device_mac, settings->mtu) < 0)
#endif
    {
...
#endif
#ifdef __APPLE__
__attribute__((visibility("default"))) int stop_edge_v2s(void)
#endif
{
    keep_running = 0;
